...
class MyApplication(QMainWindow):
    ...
    def handle_signal_plc(self, data):
        logger.info("Nhan duoc tin hieu scan tu PLC")
    ...
    def scan_product_code(self):
        ...
        if self.data_scan1 is not None and self.data_scan2 is not None:
            ...
            # !TEST : capture old sn data
            # 352, 104, 780, 136 ltrb
            # l t w h
            if self.IS_USE_SNAPSHOT_OLD_SN == 1:
                # self.old_sn = capture_result_groupbox(left=self.POSITION_SN[0], top=self.POSITION_SN[1], width=(780 - 352), height=(136-104))
                self.old_sn = capture_result_groupbox(
                    left=352,
                    top=104,
                    width=200,
                    height=32,
                )
                cv2.imwrite("./temp/old_sn.png", self.old_sn)
            ...
            if self.is_matching_1 == True and self.is_matching_2 == False:
                ...
                if self.is_matching_1 == True and self.is_matching_2 == True:
                    if self.IS_USE_SNAPSHOT_OLD_SN == 1:
                        result_compare_sn = compare_sn_template(self, self.old_sn)
                        if result_compare_sn == True:
                            cmd_printer("ERROR", "--> ERROR MES WITH SPECIAL CASE")
                            logger.info("--> ERROR MES WITH SPECIAL CASE")
    ...
    def reconnect_camera_thread(self):
        ...
        try:
            if (
                len(self.graph.get_input_devices()) == self.NUM_CAMERA
                and self.count_camera_error > 0
                and self.THREAD_CAMERA_1.isRunning()
                and self.THREAD_CAMERA_2.isRunning()
            ):
                # self.open_camera_thread()
                # stop reopen camera thread
                # trying to close application and reopen it
